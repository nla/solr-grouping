 ...
 * @lucene.experimental
 */
abstract public class AbstractFirstPassGrouping2Collector<GROUP_VALUE_TYPE> extends SimpleCollector {
  ...
  private Map<GROUP_VALUE_TYPE, CollectedSearchGroup2<GROUP_VALUE_TYPE>> groupMap;
  ...
  public AbstractFirstPassGrouping2Collector(Sort groupSort, int topNGroups) throws IOException {
    if (topNGroups < 1) {
   ...
   */
  public Collection<CollectedSearchGroup2<GROUP_VALUE_TYPE>> getTopGroups(int groupOffset, boolean fillFields) {
    ...
    final Collection<CollectedSearchGroup2<GROUP_VALUE_TYPE>> result = new ArrayList<>();
    int upto = 0;
